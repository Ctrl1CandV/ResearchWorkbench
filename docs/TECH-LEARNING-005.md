...
## 0. 2026-09-22 状态修订（SCAFFOLD-008 / 008.2，技术方案沿用008.1）

**本节以下 T1–T6「六条必学主干」的定义保留为历史/保留内容**：数据不删除、仍可点开学习，但不再是技术页的默认课表。新默认以 [SCAFFOLD-008/04](SCAFFOLD-008/04-tech-routes.md) 为准，恰好三条：

1. **多智能体架构（必学）**：Python + LangGraph 四个连续单元（单 Agent 工具循环 → 状态/检查点/恢复 → 双 Agent 分工与反馈 → 有界重试/人工接管/交接边界），共用一个「小型算术任务助手」贯通练习；唯一教材 public/learning/multiagent-lab.md，练习在独立目录运行、不接入工作台。
2. **RAG / 检索记忆（按需）**：原 T4 规范 id 改为 `tech-rag`（旧 `tech-t4` 仅作读取别名），保留 t4-u1/u2/u3 单元与已核资源，补 LlamaIndex starter 为主资源。
3. **图（浅尝）**：NetworkX 数据结构入门 + Distill GNN 直觉 + 一段站内编辑说明；不是第三条论文方向。

T1–T6 与全部支线归入「其他主干（当前不必先学）」；不要求先选题才能学；不要求按 T 系列顺序学。
旧「方向二/三」之类的编号指称以方向名称与「已延后」状态为准（见 01 §C）。

本次主要调整论文方向，不重选技术主框架和练习。研究中的长期记忆仍是候选机制，RAG工程按需不等于研究不重要；LangGraph教学不限定未来跨harness研究实现。featured的论文关联仅指实际已入库的memgpt/beyond-frameworks等，不引用未建卡的coala。具体改动与中断续做见 [09](SCAFFOLD-008/09-change-and-resume.md)。
