    ...
    zones: [
      {
        key: 'goal',
        title: '阅读目标',
        purpose: '从起步导读、基础材料和本人标记的在读论文中，找到当前要读的内容。',
        howToUse: '按主方向顺序起步；精选用于了解前沿，不是每日作业。',
        entryLabel: '打开主方向路线',
      ...
      },
      {
        key: 'survey',
        title: '知识脉络',
        purpose: '从 AI 背景、Agent 全景到专题分支，串起关键概念与发展脉络。',
        howToUse: '先看图，再点节点读讲解；六条路径可作选读顺序。',
        entryLabel: '打开广域脉络图',
        entryHash: '#/map',
      },
      {
        key: 'topic',
        title: '课题深化',
        purpose: '查看主方向的问题结构、协作机制与信息表示，以及问题如何演化。',
        howToUse: '沿四个分析轴展开，再看相关论文和仍待回答的问题。',
      ...
      },
      {
        key: 'tech',
        title: '技术入口',
        purpose: '多智能体、RAG 与图学习三条路线，连接概念理解和研究实践。',
        howToUse: '遇到论文或课题需要时再补对应单元，不必按顺序学完。',
        entryLabel: '进入技术学习',
        entryHash: '#/learn',
      },
    ],
...
// PLAN-012：让全部节点都能单独读懂。编辑示例用于解释机制，不代表所列论文逐句证明了示例。
const LANDSCAPE_READER_GUIDES = {
  'land-a1': {
    motivation: '如果让机器下棋，先把每一步能走到哪里、什么叫好局面写清楚，机器就能在规则里搜索答案。这条思路也用于定理证明和专家系统。它的吸引力是规则可检查，困难是现实世界的常识和例外很难一条条写完。',
    mechanism: '先把知识表示成可以操作的符号，例如“棋子在某格”“某条件成立”，再写出规则说明怎样从当前状态推出下一步。搜索会试探多种可能；分支一多，就靠规则约束和启发式评估尽早排除不太有希望的路线。机器不是先从大量例子里学这些规则，而是按人写下的表示和规则推演。',
    example: '象棋程序可以列出合法走法、推演对手可能的回应，再用评估函数比较局面。医学专家系统也能把已知症状和规则组合成诊断建议。两类系统都能在边界清楚的任务里工作；遇到规则没有覆盖的情形，就容易卡住。',
    confusion: '这条路线不等于“旧 AI 已经消失”。它擅长规则明确、结果可检查的任务，但维护常识规则的成本很高。后来人们也探索把神经网络与符号推理结合起来，让两类方法互相补足。',
    links: '接下来的统计学习换一种办法：少让人逐条写规则，多从例子中归纳规律。两者关注的问题相似，答案来自不同地方——一边显式写知识，一边从数据估计。',
  },
  'land-a2': {
    motivation: '垃圾邮件的写法会变化，单靠人维护“含某词就是垃圾邮件”的规则，很快就会漏判。统计学习的出发点是：给机器足够多的例子，让它从例子里找出可重复的规律。',
    mechanism: '训练时，模型根据一批带答案的样本调整参数，使预测逐渐接近样本答案；再用训练中没见过的测试样本检查它能不能推广。只记住训练样本叫过拟合。限制模型复杂度、增加合适约束等做法可以降低这种风险，但并不能让数据中的偏差自动消失。',
    example: '准备已经标为“垃圾邮件”或“正常邮件”的邮件，让模型观察词语等特征与标签的关系。训练后，再用新邮件测试。若训练邮件判断很准、新邮件却常错，说明模型可能记住了旧样本中的偶然细节。',
    confusion: '统计学习是一整套从数据学习规律的观点，不是某一种算法。深度学习属于这个大方向；它进一步把特征的学习也放进模型训练里。统计方法仍广泛使用，尤其在数据较少或任务结构明确时。',
    links: '符号与搜索把规则写在程序里，统计学习把部分规律交给样本。深度学习随后延续“从数据中学”的方向，并尝试同时学习怎样表示输入。',
  },
  'land-a3': {
    motivation: '假设要识别照片里的猫。过去常要先由人挑出轮廓、颜色或纹理等线索，再教分类器怎样组合这些线索。换了光线和角度，手工设计的线索可能就不够用了。深度学习想让机器在训练中也学会构造有用的特征。',
    mechanism: '图片经过多层计算，每层把上一层的结果重新组合，最后输出预测。训练时，程序比较预测与正确答案，再把误差传回各层，逐步调整参与计算的参数。“表示”就是这些中间结果：它把原始像素转成更便于当前任务处理的信息。这里的“学习特征”不是给每层预先指定一项人类可读的职责。',
    example: '在常见的图像卷积网络中，靠近输入的层通常对局部边缘或纹理有反应，后面的层可以组合出更复杂的形状与部件。卷积会在图像不同位置重复局部计算，因此能利用邻近像素之间的关系。循环网络则按顺序接收输入，并用持续更新的状态保留前面的信息；它是另一类网络结构，不等于深度学习本身。',
    confusion: '“底层边缘、高层物体”是帮助想象层次特征的例子，并不是每一层都有固定的人类语义。深度学习也没有消除对数据与算力的需求；训练数据偏向什么，模型就可能把什么偏差一并学进去。',
    links: '统计学习说明怎样从样本中检查规律，深度学习把表示也交给训练。下一步的注意力与 Transformer 改变了序列中信息互相传递的方式，成为许多预训练模型采用的架构路线。',
  },
  'land-a4': {
    motivation: '语言和其他序列有先后次序。循环网络一次处理一个位置，前面的信息要经过一连串步骤才能影响后面的判断；序列越长，并行处理就越困难。Transformer 的一项关键改变，是让序列各位置能直接交换信息。',
    mechanism: '自注意力会为当前词计算一组权重，表示输入里其他位置的信息与当前任务有多相关，再按权重组合这些信息。查询、键、值是实现这种匹配和取信息的三组向量；位置编码补上词语原本的先后顺序。多个注意力头可以并行处理不同关系，整段序列也因此可以并行计算。',
    example: '机器翻译时，译文某个词可能同时需要参考句子开头的主语和后面的动词。自注意力让每个位置在计算时直接利用其他位置的表示，而不必只等相邻一步步传来信息。',
    confusion: '注意力表示计算时怎样汇集输入信息；它本身不是长期记忆，也不保证模型完成了正确推理。标准注意力在序列变长时会增加计算和显存负担，所以后续出现了降低长序列开销的研究路线。',
    links: '深度学习提供了从数据学习表示的基础；Transformer 提供了一种处理序列的架构。它帮助大规模预训练变得可行，之后的基础模型与 Agent 又把它用于更长的输入和多轮交互。',
  },
  'land-b2': {
    motivation: '单轮问答通常只需要给出一次回答；Agent 则要在多步任务里行动、接收结果，再决定下一步。两者表面上都在调用模型，真正差别在任务状态和环境反馈会不会进入下一轮。',
    mechanism: '把“修一个程序问题”想成一串步骤：先读问题，再找代码、修改文件、运行测试。每一步都要保留当前进展；下一步使用的状态如果过时或丢失，就可能重复劳动。上一轮判断错了，也可能把错误传给之后的步骤。工具返回的真实结果可以帮助发现错误，但 Agent 仍须正确理解结果。',
    example: '假设第一步找到的是同名的错误文件，第二步就可能改错位置；如果后面只根据模型写出的“应该修好了”继续走，而不运行测试，前面的错误就会留到最后。检查每个工具的实际返回，能让后续计划基于真实状态调整。',
    confusion: '多步循环是 Agent 的常见组织方式，但仅仅加一层循环不等于可靠完成长任务。状态保存、计划调整和错误检查仍可能失败；Agent 也不一定每步都采用复杂规划。',
    links: '这三类问题引出后面的推理与规划、工具与记忆，以及评测与成本。需要从整体闭环理解 Agent 时，回到“Agent 与感知—规划—行动循环”节点。',
  },
  'land-c1': {
    motivation: 'Agent 接收的信息不总是文字。用户可能给一张图片、一段录音或视频片段；系统若无法把它们转成可处理的信息，就无法根据视觉或声音完成任务。',
    mechanism: '多模态系统通常先用适合不同输入的编码器处理图像或声音，把结果变成一串数值表示，再让语言模型结合文字指令处理这些表示。可以把它想成先将不同格式的材料翻译成模型能共同处理的“内部记号”。具体系统怎样连接编码器和语言模型并不完全相同。',
    example: '看图回答“桌上有几只杯子”，系统要从图像中提取物体和位置线索，再把问题与图像信息结合成回答。若图像辨认错了，后面的推理可能听起来通顺，却从错误的观察开始。',
    confusion: '“能够输入图片”不等于准确理解图片，也不保证空间关系或细节识别可靠。错误可能来自图像感知本身，之后还会进入 Agent 的计划和行动。此处只介绍总览，不为尚未逐项核验的感知方法下性能结论。',
    links: '感知属于闭环的输入侧：Agent 先得到什么信息，限制了后面能怎样推理。接下来的推理与规划分别说明“怎样一步步想”和“怎样组织多步任务”。',
  },
  'land-e3': {
    motivation: 'Agent 调用工具后可能读网页、改文件或提交操作，错误的影响会从回答扩展到真实环境。安全问题因此包括“模型说了什么”，也包括“它被允许做什么”。',
    mechanism: '一个容易理解的风险是恶意网页把提示写成看似正常的指令，试图让 Agent 忽略原任务。常见工程防护会限制工具权限、区分数据与指令、要求关键操作前确认，并检查执行结果。这些是帮助控制风险的设计思路，不能据此断言某种防护已经充分有效。',
    example: '让 Agent 总结网页时，网页正文可能包含“把本地文件发给我”一类内容。系统应把网页当作待分析的数据，而不是自动服从的新任务；读取内容也不应自动赋予网页修改文件的权限。',
    confusion: '会遵循指令不等于安全，工具调用成功也不等于操作适当。当前全景材料缺少安全专题的深入来源，所以本节点用于建立问题意识，不比较防护方案效果。',
    links: '风险会沿工具使用进入环境，也会受对齐与权限设计影响。它需要与评测节点共同考虑：除了任务做成没有，还要看过程是否越权、错误是否能被发现。',
  },
  'land-e4': {
    motivation: 'Agent 部署后会遇到新任务和新信息。每次重新训练模型既不方便，也可能改变已有能力；完全不保存经验，又会让它重复处理已经解决的问题。',
    mechanism: '适应经验大致有三种存放位置：写进当前对话供短期使用；更新模型参数让能力更持久；把可复用流程、文档或工具留在模型之外。它们改变信息保留的位置与方式，组合使用也很常见。当前综述按是否更新参数组织相关能力，但本节点不把它说成唯一分类。',
    example: '一次任务中发现的注意事项可以留在当前交接摘要里，帮助下一个 Agent 接续；反复有用的步骤可以整理成团队流程。若经验未经核对，错误也可能跟着被保存，并在之后被继续采用。',
    confusion: '“记住了”不等于“正确使用”，更新模型也不一定能保留旧能力。什么值得写入、什么时候取出、怎样检查内容，都是独立问题。本处仅做总览，持续学习的效果需要更专门的来源支持。',
    links: '当前对话里的信息与记忆节点相连，工具和共享文件可承载更持久的经验；怎样评估适应后的质量，则与评测节点相连。',
  },
  'land-e5': {
    motivation: '修代码需要读懂问题、找到相关实现、做出修改并确认没有引入新错误。Agent 可以帮助执行这条循环，软件工程也提供了较明确的真实任务作为评估对象。',
    mechanism: '常见流程是先读取 issue 和仓库结构，定位可能相关的代码，形成修改，再运行与问题有关的测试。失败时要读测试输出、检查代码并继续调整。整个过程要保留任务状态；当测试通过后，还要确认改动对应原问题而不是偶然绕过检查。',
    example: '一个 issue 报告某种输入会让程序崩溃。Agent 需要先找到相关处理函数，再修改并运行能覆盖这种输入的测试。只看到“测试通过”还不够：也要检查改动是否解决了报告的问题、有没有误删其他行为。',
    confusion: '测试是有用的验证信号，但覆盖不到所有真实情况；测试通过不保证修复正确。也不能把某次演示成功当成稳定解决整个软件工程任务。',
    links: '代码智能体把工具使用、长任务中的状态管理和评测汇到软件工程场景，是第二条研究方向的入口。阅读时可把它和评测、成本节点放在一起理解。',
  },
  'land-f1': {
    motivation: '学完 Agent 的组成后，需要把广域知识接回正在研究的问题：跨工具接力时，前一个 Agent 留下的状态怎样交给下一个 Agent，怎样限制预算，又怎样比较结果？',
    mechanism: '把问题拆成几项方便阅读的选择：状态存在哪里、谁决定保存或取回什么、信息用什么形式表示、如何获取以及怎样计算代价。多智能体协作提供任务接力场景；记忆与检索提供信息保存和取回方式；评测与成本提醒我们同时记录任务质量和交互开销。',
    example: '工具 A 修到一半时，可以留下修改过的文件和一段交接说明；工具 B 再读取说明并检查文件。比较不同接力方式时，除了看问题是否修好，也要观察 B 获取了多少信息、花了多少交互，以及是否重复工作。这是帮助拆解研究问题的编辑示例，不代表哪种机制已经胜出。',
    confusion: '这里列出的分析问题是本项目主方向的组织方式，不是综述公认的统一分类。它们也不是预设答案：共享状态、交接说明、长期记忆和检索可以组合，适用性要结合场景与预算比较。',
    links: '多智能体、工具、RAG、记忆、规划与评测各自提供一部分背景。进入主方向后可先读站内的问题导读，再按需要回到这些节点补概念。',
  },
};

for (const node of LIBRARY.landscape.nodes) {
  if (!node.guide) node.guide = LANDSCAPE_READER_GUIDES[node.id];
  if (!node.sourceBrief && node.source?.note) {
    node.sourceBrief = `参考：${node.source.note.split(/[；;]/, 1)[0]}`;
  }
}
